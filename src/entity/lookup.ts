...
export function isPosOccupied(
    pos: Vector2Like,
    state: GameState,
    offset = 0,
    ignoreEntities?: EntityId[],
): boolean {
    for (const entity of iterateCollidable(state)) {
        if (ignoreEntities?.length && ignoreEntities.includes(entity.id)) continue;
        if (isPosInsideRect(pos.x, pos.y, entity, offset)) {
            return true;
        }
    }
    return false;
